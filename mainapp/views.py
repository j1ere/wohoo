...
def group_chat(request, group_name):
    user = request.user
    if Group.objects.filter(members=user, name=group_name).exists():
    ...
    return JsonResponse({"message": "you are not a member of this group"}, status=403) #forbiden acccess attempt
